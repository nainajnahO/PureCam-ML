...
enum DataFrameBuilder {
    /// Add all 14 scene feature columns to a DataFrame
    /// - Parameters:
    ///   - dataFrame: The DataFrame to modify (passed as inout)
    ///   - features: Array of feature arrays, where each inner array has 14 Float values
    static func addFeatureColumns(to dataFrame: inout DataFrame, features: [[Float]]) {
        ...
        dataFrame.append(column: Column(name: "sceneLightLevel", contents: features.map { $0[13] }))
    }

    /// Create DataFrame for ISO model training (scene features → log2 ISO target)
    /// - Parameters:
    ///   - features: Array of 14-element Float arrays (scene features)
